...
6. **Trạng thái dẫn xuất** trong `NhanKhauResponseDto.trangThaiHienTai`:
   - Logic: service kiểm tra các khoảng ngày tạm trú/tạm vắng so với ngày hôm nay (`LocalDate.now()`).
   - Độ ưu tiên: `TAM_VANG` > `TAM_TRU` > `THUONG_TRU`.
   - `TAM_VANG` nếu hôm nay thuộc [`tamVangTu`, `tamVangDen`] (cả hai không null và hôm nay nằm trong khoảng).
   - Ngược lại `TAM_TRU` nếu hôm nay thuộc [`tamTruTu`, `tamTruDen`].
   - Nếu không thuộc khoảng nào thì `THUONG_TRU`.
   - Trường này **không lưu trong CSDL**, chỉ tính khi trả về DTO.
...
### 6.1 Danh sách loại biến động (`BienDongType`)
| Loại | Mô tả |
|------|--------|
| `CHUYEN_DEN` | Nhân khẩu chuyển đến hộ khẩu. |
| `CHUYEN_DI` | Nhân khẩu chuyển đi khỏi hộ khẩu. |
| `TACH_HO` | Tách hộ khẩu. |
| `NHAP_HO` | Nhập hộ khẩu. |
| `SINH` | Đăng ký khai sinh. |
| `KHAI_TU` | Đăng ký khai tử. |
| `THAY_DOI_THONG_TIN` | Thay đổi thông tin nhân khẩu. |
| `TAM_TRU` | Đăng ký tạm trú. |
| `HUY_TAM_TRU` | Hủy đăng ký tạm trú. |
| `TAM_VANG` | Đăng ký tạm vắng. |
| `HUY_TAM_VANG` | Hủy đăng ký tạm vắng. |

### 6.2 Quy tắc ghi nhật ký
1. Controller chỉ chấp nhận các loại viết hoa cố định (xem bảng 6.1).
