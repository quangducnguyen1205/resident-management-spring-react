...
| **Quản lý đợt thu phí** | Toàn quyền | Chỉ xem | Toàn quyền |
| **Thu phí hộ khẩu** | Toàn quyền | Chỉ xem | Toàn quyền |
| **Quản lý tài khoản** | Toàn quyền | Không | Không |

> **Lưu ý:** TOTRUONG chỉ có quyền quản lý hộ khẩu và nhân khẩu. KETOAN chỉ có quyền quản lý các module thu phí.
...
| Method | Endpoint | Mô tả | Phân quyền |
|--------|----------|-------|------------|
| GET | `/api/dot-thu-phi` | Lấy danh sách tất cả đợt thu phí | ADMIN, TOTRUONG, KETOAN |
| GET | `/api/dot-thu-phi/{id}` | Lấy thông tin chi tiết một đợt thu phí | ADMIN, TOTRUONG, KETOAN |
...
| Method | Endpoint | Mô tả | Phân quyền |
|--------|----------|-------|------------|
| GET | `/api/thu-phi-ho-khau` | Lấy danh sách tất cả bản ghi thu phí | ADMIN, TOTRUONG, KETOAN |
| GET | `/api/thu-phi-ho-khau/{id}` | Lấy thông tin chi tiết một bản ghi | ADMIN, TOTRUONG, KETOAN |
| GET | `/api/thu-phi-ho-khau/ho-khau/{id}` | Lấy lịch sử thu phí của một hộ khẩu | ADMIN, TOTRUONG, KETOAN |
| GET | `/api/thu-phi-ho-khau/dot-thu-phi/{id}` | Lấy danh sách thu phí của một đợt | ADMIN, TOTRUONG, KETOAN |
| GET | `/api/thu-phi-ho-khau/calc` | Tính toán phí cho hộ khẩu | ADMIN, TOTRUONG, KETOAN |
| GET | `/api/thu-phi-ho-khau/stats` | Thống kê tổng quan | ADMIN, TOTRUONG, KETOAN |
...
- **Hộ khẩu và đợt thu phí**: Bắt buộc, phải là số dương.
- **tongPhi**: Chỉ bắt buộc với đợt tự nguyện (`TU_NGUYEN`) và phải > 0. **Không được gửi** trường này với đợt bắt buộc (`BAT_BUOC`).
- **Ngày thu (`ngayThu`)**: **Bắt buộc** cho cả hai loại đợt thu và phải nằm trong khoảng `ngayBatDau` – `ngayKetThuc` của đợt thu phí. Thiếu trường này sẽ trả về `400 Bad Request`.
- **ghiChu**: Tùy chọn.
